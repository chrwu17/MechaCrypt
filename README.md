...
- `sender/`: Contains the FPGA and MCU code for the sender side of the MechaCrypt system.
  ...
  - `mcu/`: Contains C code implementing Web server for message input and parsing, TRNG for encryption key generation, SPI communication with FPGA to send keys, plaintext and cyphertext.

- `receiver/`: Contains the FPGA and MCU code for the receiver side of the MechaCrypt system.
  - `fpga/`: Contains SystemVerilog code for implementing AES decryption.
