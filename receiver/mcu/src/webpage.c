...
// ----------------- Shared state -----------------
volatile uint8_t received_blocks[MAX_BLOCKS][16];  // Decrypted blocks from FPGA
volatile uint8_t have_received[MAX_BLOCKS];        // Which blocks have been received
volatile uint16_t total_received = 0;              // Total blocks received
...
// ----------------- Helper to convert blocks to JSON -----------------
...
// ----------------- Public: Store received block from FPGA -----------------
void store_received_block(int block_idx, const uint8_t *block16) {
  ...
  led_blink_short();  // Visual feedback
}
...
void processWebRequest(USART_TypeDef *USART)
{
  ...
  // Handle: GET /data - Return JSON with all received blocks
  if (strstr(request, "/data") || strstr(request, "data")) {
    send_json_data(USART);
    return;
  }

  // Handle: GET /clear - Clear all received data
